...
const src = fs.readFileSync('assets/vim.js', 'utf8').replace(/^export default Vim;$/m, '');
const Vim = eval(src + '\nVim');
...
t('gcc round-trips',      'x = |1',          'gccgcc',   '|x = 1');
t('2gcc takes two lines', '|a\nb\nc',        '2gcc',     '|# a\n# b\nc');
t('gcc keeps indent',     '    x =| 1',      'gcc',      '    |# x = 1');
...
console.log('\n--- the preference survives, which is the whole point ---');
{
  ...
  Vim.setOn(true);
  ok('turning it on writes to storage', store['ph.vim'] === '1', JSON.stringify(store));
  ok('and reads back on', Vim.isOn() === true);
  Vim.setOn(false);
  ok('turning it off persists too', Vim.isOn() === false && store['ph.vim'] === '0');
