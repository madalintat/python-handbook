...
`items` and `data` are two names for one list. `clear()` mutates it, and the caller sees the result. This is not "pass by reference" in the C++ sense, and it is not "pass by value" either: the value passed is a reference, which people usually shorten to *pass by object reference* or *call by sharing*.
